...
void SynthVoice::renderNextBlock (juce::AudioBuffer <float> &outputBuffer, int startSample, int numSamples)
{
    if (_noteOn) {
        for (int i = startSample; i < (startSample + numSamples); i++) {
            ...
            float pos = wavetable::sel->size() * (elapsedTime / _period - std::floor(elapsedTime / _period));
            ...
            if (lp == rp) {
                v = wavetable::sel->at(lp);
            }
            else {
                lv = wavetable::sel->at(lp);
                /* avoid overflow */
                rv = (rp == wavetable::sel->size()) ? wavetable::sel->at(0) : wavetable::sel->at(rp);
